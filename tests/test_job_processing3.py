...
NOTE: the tests are not finished.
...
import pysimserv3 as pysimserv
...
class TestProcessJob(unittest.TestCase):
    ...
    def setUp(self):
        self.log = MockLog()
        self.m = MockMessage()
        self.oldDeltas = pysimserv.conf["DELTAS"]
        self.oldGridStep = pysimserv.conf["grid_step"]

        # also save PARAMS if needed in the future
        self.oldSUM_SOURCES = pysimserv.conf['SUM_SOURCES']

    def tearDown(self):
        pysimserv.conf["DELTAS"] = self.oldDeltas
        pysimserv.conf['SUM_SOURCES'] = self.oldSUM_SOURCES
        pysimserv.conf["grid_step"] =  self.oldGridStep
    ...
    def test_case0(self):
        ...
        pysimserv.conf["grid_step"] = 1
        result = pysimserv.process_job(ds, self.m, self.log)
        true_result = ds.true_results()
        self.assertAlmostEqual(result[0], true_result[0])
        self.assertAlmostEqual(result[1]['user'][0], true_result[1], delta=0.00000001)
        self.assertAlmostEqual(result[1]['user'][1], true_result[2], delta=0.00000001)

    def test_case2points(self):
        """
        simple one component of 2 records
        """
        ds = MockDataSourceCase2points()
        # pysimserv.conf["grid_step"] = 1
        result = pysimserv.process_job(ds, self.m, self.log)
        true_result = ds.true_results()
        self.assertAlmostEqual(result[0], true_result[0])
        ...
        # now with a different grid step
        ds = MockDataSourceCase2points()
        pysimserv.conf["grid_step"] = 1
        result = pysimserv.process_job(ds, self.m, self.log)
        true_result = ds.true_results()
...
class MockDataSource:
  ...
  def select(self, colums, where = None, order_by = None, desc = False):
    self.desc = desc
...
class MockDataSourceCase0(MockDataSource):
  ...
  def __init__(self):
    ...
    self.values = np.cumsum(self.deltas) + 10
    self.times = np.arange(self.starttime, self.starttime + np.timedelta64(self.N, 's'), dtype='datetime64[s]')
    self._reset()
  ...
  def _get_row(self):
    if self.cur_record >= self.N:
      ...
      record = self.N-1 - self.cur_record if self.desc else self.cur_record
      t, c, u = self.times[record], 1, self.values[record]
      self.cur_record += 1
      return t, c, u
...
class MockDataSourceCase2points(MockDataSource):

  def __init__(self):
    MockDataSource.__init__(self)
    self.N = 2
    self.values = [10, 15]
    self.times = [self.starttime, self.starttime + np.timedelta64(5, 's')]
    self._reset()
